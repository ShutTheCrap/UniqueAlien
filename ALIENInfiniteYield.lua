--[=[
  ...
  Alien version of Infinite Yield
  Maintained by Unique#9468
...
PARENT = nil
if (not is_sirhurt_closure) and (syn and syn.gethui) then --sirhurt is retarded
...
Title.TextSize = 18
Title.Text = "IY Synapse V3 Edition "..ver --//too long text
...
Credits.Text = "Edge // Zwolf // Moon // Toon // Ender"
...
About.TextSize = 14
About.Text = "Plugins are _IY.lua files and should be located in the 'workspace' folder of your exploit."
...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("IY_FE.json") end) then
			if readfile("IY_FE.json") ~= nil then
				local success, response = pcall(function()
					local json = game:GetService("HttpService"):JSONDecode(readfile("IY_FE.json"))
				...
				if not success then
					...
					writefileCooldown("IY_FE.json", defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("IY_FE.json", game:GetService("HttpService"):JSONEncode(update))
	end
end
...
task.spawn(function()
	if pcall(function() loadstring(game:HttpGet('https://raw.githubusercontent.com/DaEnder/Infinite-Yield-V3-Edition/main/version.lua'))() end) then
		if ver ~= Version then
			notify('Outdated','Get the new version at the SX 3.0 Discord.')
		end
